...
def generate_report(cur_curator_login):
   try:
      ...
      ws["B2"] = ws["B2"].value.replace("{{ curator_name }}", curator.name)
      ws["B3"] = ws["B3"].value.replace("{{ count }}", curators_employees.count())
      ...
      for employee in curators_employees:
         ...
         employees_polls = Poll.objects.filter(question__answer__login = employee.login).distinct()
         for employee_poll in employees_polls:
            ...
            employees_questions = Question.objects.filter(poll = employee_poll.id)
            ...
            for employees_question in employees_questions:
               ...
               question_answer = Answer.objects.filter(question = employee_poll.id, login = employee.login).first()   
               #заполняем строку       
      ...
      #сохранение отчета
      report_filename = f"report_{cur_curator_login}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
