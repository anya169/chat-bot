...
from core.models import Employee, Answer, Poll, BotSendPoll
...
async def finish_poll(message: Message, state: FSMContext, question_id=None):
    ...
    async with ChatActionSender.typing(bot = bot, chat_id = message.chat.id):
        ...
        await message.answer(
            ...
            "Если потребуется дополнительная консультация или разъяснения, куратор непременно выйдет на связь. Спасибо за сотрудничество и успехов тебе! 🚀"
        )
    async with ChatActionSender.typing(bot = bot, chat_id = message.chat.id):
