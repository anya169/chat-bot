...
async def capture_curator_information(message: Message, state: FSMContext):
    ...
    async with ChatActionSender.typing(bot = bot, chat_id = message.chat.id):
        ...
        await message.answer("В нашей компании адаптация новых работников является приоритетной задачей, "
                             "ведь люди — наша главная ценность.\n\n"
                             "На протяжении периода адаптации рядом с тобой будет куратор, "
                             "готовый помочь разобраться в любых возникающих вопросах и обеспечить выполнение всех необходимых процедур"
                             "согласно внутренним правилам и стандартам компании. \n\n"
                             "Помни, ты не просто начинаешь карьеру здесь — ты становишься частью команды профессионалов, которые строят будущее нашей организации!")
    async with ChatActionSender.typing(bot = bot, chat_id = message.chat.id):
        await asyncio.sleep(long_delay)
...
async def capture_task(message: Message, state: FSMContext):
    async with ChatActionSender.typing(bot = bot, chat_id = message.chat.id):
        await asyncio.sleep(short_delay)
        await message.answer("Внутренними правилами компании предусмотрено обязательное "
                             "прохождение психологического тестирования всеми молодыми специалистами. \n\n"
                             "Чтобы пройти тест, перейди по указанной ниже ссылке: https://forms.yandex.ru/cloud/665e7565d046880010428699/\n\n"
                             "По итогам обработки данных, ты получишь обратную связь с рекомендациями по совершенствованию. После завершения тестирования вернись в диалог и подтверди выполнение, нажав кнопку \"Я заполнил(а)\".", 
